...
public class BoxController : MonoBehaviour {
    ...
    private void Start () {
        ...
        for (int i = 0; i < m_BoxList.Count; i++) {
            ...
            m_BoxList[i].transform.localPosition = GameMath.GetPositionWithRadius (transform.position, BezierPathGen.PathRadius * 1.01f, angleInRad);
        }
    ...
    private void RotateBoxes () {
        ...
        m_BoxList[0].transform.localPosition = GameMath.GetPositionWithRadius (
            Vector3.zero, BezierPathGen.PathRadius * 1.5f, firstAngle
        );
        m_BoxList[1].transform.localPosition = GameMath.GetPositionWithRadius (
            Vector3.zero, BezierPathGen.PathRadius * 1.5f, secondAngle
        );
